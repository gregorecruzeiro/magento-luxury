...
class Mage_Wishlist_IndexController extends Mage_Wishlist_Controller_Abstract
{
    ...
    protected function _addItemToWishList()
    {
        ...
        try {
            ...
            $message = $this->__('%1$s adicionado ao favoritos. Clique <a href="%2$s">aqui</a> para continuar comprando.',
                $product->getName(), Mage::helper('core')->escapeUrl($referer));
            $session->addSuccess($message);
        } catch (Mage_Core_Exception $e) {
            $session->addError($this->__('Ocorreu um erro ao adicionar ao favoritos: %s', $e->getMessage()));
        }
        catch (Exception $e) {
            $session->addError($this->__('Ocorreu um erro ao adicionar ao favoritos.'));
        }
