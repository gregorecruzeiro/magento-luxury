<?php 
class Cmsmart_ThemeSetting_Model_Cssconfig_Generator extends Mage_Core_Model_Abstract{ 
    ...
    protected function generateStoreCss($type, $storeCode){
        ...
        $str1 = '_' . $storeCode;
        $str2 = $type . $str1 . '.css';
        $str3 = Mage::helper('themesetting/cssconfig')->getCssConfigDir() . $str2;
        $str4 = 'css/' . $type . '.phtml';
        ...
        try{ 
            ...
            if (empty($block)) {
                throw new Exception( Mage::helper('themesetting')->__("Template file is empty or doesn't exist: %s", $str4) );
            }
            $file = new Varien_Io_File(); 
            $file->setAllowCreateFolders(true); 
            $file->open(array( 'path' => Mage::helper('themesetting/cssconfig')->getCssConfigDir() )); 
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('themesetting')->__('Failed generating CSS file: %s in %s', $str2, Mage::helper('themesetting/cssconfig')->getCssConfigDir()). '<br/>Message: ' . $e->getMessage());
            Mage::logException($e);
        }
