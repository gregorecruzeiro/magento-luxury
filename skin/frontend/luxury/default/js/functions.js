(function($)
{
	...
	$(window).scroll(function()
	{
		...
		if( scroll >= 300 && width >= 992 )
		{
			$('.lx-cart-mobile').addClass("fixed-cart-mobile")
		}
		else if ( scroll <= height )
		{
	...
	$(document).ready(function($)
	{
		...
		/* Price Filter */
		var currency_rate = parseFloat($('span#lx-f-cur-rate').html());
		var curr_rate = Math.round(currency_rate * 1000)/1000;
		var price_max = Math.round(parseFloat($('span#lx-f-price-max').html()));
		var price_values = Math.round(price_max*2/3);
		$( "#slider-range" ).slider({
			range: true,
			min: 0,
			max: price_max,
			values: [ 0, price_values ],
			slide: function( event, ui ) {
				...
				var link = dest;
				var link1 = dest;
				var index_p = dest.indexOf(str);
				if (index_p != -1) {
					var link1 = dest.slice(0, index_p);
					var link2 = dest.slice(index_p);
					var index_a = link2.indexOf(str1);

					if(index_a == -1) {
						link = link1;
					}else {
						var link3 = link2.slice(index_a);
						link = link1.concat(link3);
					}
				}
				link += (window.location.href.indexOf('?') != -1) ? '&' : '?' ;
				link += 'price=' + minvalue + '-' + maxvalue;
				//alert(link);alert(link2);
				window.location = link;
			}
		});
		var currency = $('span#lx-f-cur').html();
		$( "#amount" ).val( $( "#slider-range" ).slider( "values", 0 ) + " " +currency );
		$( "#amount2" ).val( $( "#slider-range" ).slider( "values", 1 ) +  " " + currency );
		$( "#amount2" ).keypress(function(e) {
			if(e.keyCode == 13) {
				...
				var minvalue2 = parseFloat($( "#amount" ).val().replace( /^\D+/g, ''));
				var maxvalue2 = parseFloat($( "#amount2" ).val().replace( /^\D+/g, ''));
				link += (window.location.href.indexOf('?') != -1) ? '&' : '?' ;
				link += 'price=' + minvalue2 + '-' + maxvalue2;
				//alert(minvalue2);alert(maxvalue2);
				window.location = link;			
			}	
		} );
		...
		/* Single product */
		$( 'li.color-widget a' ).on( 'click', function(e) {
			$( "li.color-widget a" ).removeClass( 'active' );
			$(this).addClass( 'active' );
			return false;
		});
		
		$( 'li.size-widget a' ).on( 'click', function(e) {
			$( "li.size-widget a" ).removeClass( 'active' );
			$(this).addClass( 'active' );
			return false;
		});
